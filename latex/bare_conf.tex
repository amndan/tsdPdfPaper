...
\begin{figure}
	\begin{tikzpicture}
	\pgfmathsetmacro{\ah}{0.35}%
	\pgfmathsetmacro{\aw}{0.3}%
	\pgfmathsetmacro{\ac}{20}%
	\pgfmathsetmacro{\ir}{8}%
	\pgfmathsetmacro{\sp}{4}%
	...
	\foreach \x in {0,1,...,\ac}
	{
		\draw (\x*\aw cm,0) -- (\x*\aw cm,\ah);
	}
	\node [] (txtModel) at (0,\ah/2) [] {};
	\draw [fill=yellow] (\aw*\ir,0) rectangle (\aw*\ir+\aw,\ah);
	\end{scope}
	...
	\node [label={[label distance=0.1cm]180:Model}] at (txtModel) {};
	\node [label={[label distance=0.1cm]180:Szene}] at (txtSzene) {};
