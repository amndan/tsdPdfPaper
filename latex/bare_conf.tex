...
With the information of normal vectors it is possible to calculate a transformation ($T_{ij}$) between $M$ and $S$ taking only the information of one scan point from each dataset ($m_i$, $s_j$) into consideration. Calculation of $T_{ij}$ is as follows\todo{right calculation}:
...
RNM searches for matching point pairs in $M$ and $S$. In section XY we introduce a rating function based on the actual map representation in TSD space ($m_{TSD}$) to estimate the quality of each calculated transformation ($T_{ij}$). This rating function is denoted as $f_{rating}(T_{ij},m_{TSD})$.
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\Function{$f_{RNM}$}{$M$, $S$, $trials$, $span$, $x_{t-1}$}
