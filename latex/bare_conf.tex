...
\begin{algorithm}
	...
	\label{alg:rnm}
	\begin{algorithmic}[1]
		\Function{$f_{RNM}$}{$M$, $S$, $trials$, $span$}
...
Whereas $x$ stands for the pose of the robot, $m$ represents the actual map and $z$ is the actual measurement, in this case the actual range measurements from the laser scanner. It is common to thread each scan ray statistically independent from each other and multiply the probabilities to get one scans measurement probability (see equation~\ref{equ:multProb}).
...
Substituting variables and calculating one particular probability with given values leads to the rating function used by RNM algorithm:

\begin{align}
\label{equ:ratingFunc}
f_{rating} &\left (  T_{ij}, T_{t-1}, m_{TSD}, C, p_r\right ) = \\
\prod_k p &\left ( z_k = C[k]|x = T_{t-1} \cdot T_{ij} ,m = m_{TSD} \right ) \nonumber.
\end{align}

The measurement $z$ is represented as the control set $C$, a subset from the scene point cloud $\left(C \subset S\right)$. The map $m$ is represented as TSDFs called $m_{TSD}$. Whereas the position $x$ is represented as a transformation matrix calculated with $T_{ij}$ from RNM-Algorithm and the last known transformation returned by SLAM algorithm, which is called $T_{t-1}$.
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\State $rating = rating * tsdpf$ \Comment multiply all rays 
